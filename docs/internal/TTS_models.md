...
### Q1 (2026-05-11): Can I really test all model capabilities via Hugging Face Spaces or vendor demos? I'm worried that when I deploy locally on my own data I'll find limits the hosted demos hid.

Valid concern — and yes, hosted demos do hide most knobs, especially for TTS where there are many (voice cloning, emotion tags, cfg/exaggeration, temperature, speed/pitch, SSML, prompt engineering for Parler/CosyVoice). Don't pick one testing method — use the right tier for the question:

| Tier | Best for | Limitation |
|---|---|---|
| 1. HF Space / vendor demo | First-listen, ear-test | Parameters hidden, no real latency, often no reference-audio upload |
| 2. Hosted API (Replicate, HF Inference, Modal, Baseten, fal.ai) | Capability testing with full params | Can't change runtime; network latency in the loop; cost per call |
| 3. Colab / Jupyter notebook | Full code + every parameter, custom data | Colab VM overhead; not your real deployment env |
| 4. Local deployment | Real latency, real VRAM, choice of runtime (vLLM / CTranslate2 / ONNX / GGUF), batching, quantization | Setup pain, GPU required, large downloads |

**Key insight:** you don't need local deployment to evaluate **capabilities** — only to evaluate **production fitness** (true latency, cold start, VRAM, throughput on your target hardware). Recommended Phase-2 workflow:

1. Tier 1 to eliminate models that fail on your input style.
2. Tier 2 to verify capability claims (voice cloning, emotion tags, long-form) for survivors.
3. Tier 3 (notebooks) for top 3–4 finalists — generate side-by-side samples with all knobs tuned.
4. Tier 4 (local) for the 1–2 you're seriously deploying — measure real latency/VRAM.

**Capability coverage by tier (TTS specifically):**

| Capability | Tier 1 | Tier 2 | Tier 3 | Tier 4 |
|---|---|---|---|---|
| Reference voice cloning | Sometimes | ✅ | ✅ | ✅ |
| Emotion tags / non-verbal sounds | Usually | ✅ | ✅ | ✅ |
| Streaming TTFB (real number) | ❌ | ⚠️ + network | ⚠️ Close | ✅ |
| Cfg / exaggeration / temperature | Rarely | Usually | ✅ | ✅ |
| Speed / pitch / SSML | Rarely | Sometimes | ✅ | ✅ |
| Long-form stability | ❌ Time-capped | ⚠️ Quota | ✅ | ✅ |
| Cold-start / VRAM / throughput | ❌ | ❌ | ❌ | ✅ only here |

**Architectural tip for the plug-and-play UI (Phase 2):** make each model slot accept either a local model or a hosted-API endpoint behind a uniform interface. Same UI, same eval harness, swap backends as you progress through the tiers. This is exactly the pattern LiveKit Agents and Pipecat use — STT/LLM/TTS providers behind one interface, agent code unchanged across swaps.

### Q2 (2026-05-11): Are LiveKit and Pipecat open source?

Yes — both genuinely open source with permissive licenses, plus Vocode in the same category.

| Framework | License | Repo |
|---|---|---|
| LiveKit Agents | **Apache 2.0** | https://github.com/livekit/agents |
| Pipecat (Daily.co) | **BSD-2-Clause** | https://github.com/pipecat-ai/pipecat |
| Vocode | **MIT** | https://github.com/vocodedev/vocode-core |

**Business model pattern:** the framework is fully open and self-hostable. The companies make money by selling the **managed WebRTC / telephony / scaling infrastructure** that's painful to run yourself (LiveKit Cloud, Daily.co's hosted service) — not by paywalling the framework. For Phase 2 prototyping, run everything locally with zero licensing concern. Scale to managed only if/when concurrent users justify it.

**Why this matters for the benchmark UI:**
- All three ship **plug-in adapters for major STT/TTS/LLM providers** (Whisper, Deepgram, Cartesia, ElevenLabs, OpenAI, Anthropic, NVIDIA, etc.) — model swaps are config changes, not code rewrites.
- All three handle **VAD, turn detection, barge-in, and interruption** natively — saves weeks of plumbing.
- All three support **OpenAI Realtime API as an alternative end-to-end path**, enabling cascaded-vs-end-to-end A/B tests without rewriting agent code.
