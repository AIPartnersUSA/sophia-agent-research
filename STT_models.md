...
Conversation log of questions asked while reviewing this report, with concise answers. Latest at the bottom.

### Q1 (2026-05-11): Did the research agent use the Hugging Face Open ASR Leaderboard as the reference framework?

Yes. It is the spine of this report — every model is anchored to its WER and RTFx on this leaderboard so all entries are apples-to-apples. Vendor-specific claims (multilingual coverage, streaming latency, hardware footprint) are layered on top. The leaderboard standardizes evaluation over 10 datasets (LibriSpeech clean/other, AMI, Earnings22, SPGISpeech, TED-LIUM v3, GigaSpeech, FLEURS, MLS, CoVoST-2, VoxPopuli) on a single NVIDIA A100-SXM4-80GB. Without a shared benchmark, Whisper vs Parakeet vs Granite comparisons would be meaningless because each vendor reports their own numbers on their own test sets.

### Q2 (2026-05-11): What is the paper at arXiv:2510.06961?

It is the **paper that describes the Open ASR Leaderboard itself**, not a paper introducing a new model. arXiv ID format `YYMM.NNNNN` → "25" = year 2025, "10" = October. Written by the Hugging Face audio team (Srivastav, Gandhi, et al.), it documents the leaderboard's methodology (dataset selection, WER text-normalization rules, RTFx measurement on a fixed A100), surveys all models currently ranked with WER + RTFx numbers (this is the source of the Table 3 / Table 4 figures cited throughout the report), and analyzes architectural trends (e.g. "top 4 English models all use Conformer encoder + LLM decoder"). It is the authoritative, peer-reviewable source — citing it instead of the live web page means the claims won't rot if the leaderboard UI changes. Links: https://arxiv.org/abs/2510.06961 (abstract) · https://arxiv.org/html/2510.06961 (HTML) · code https://github.com/huggingface/open_asr_leaderboard.

### Q3 (2026-05-11): WER = is speech correctly captured as text? RTFx = latency in short?

WER ✅ correct — it measures **accuracy**. Sum of substitutions + insertions + deletions divided by total reference words. Lower = better. WER 2% means 2 of every 100 words are wrong.

RTFx is **speed / throughput, not strictly latency**. It answers "how many seconds of audio can the model transcribe in 1 second of wall-clock time?" Higher = faster. RTFx = 1 is real-time, RTFx = 100 is 100× faster than real-time, RTFx = 3,386 (Parakeet-TDT-v2) is blazing batch throughput. Latency in the voice-agent sense is "how long until the first word appears after the user speaks" — that is a *different* number, captured separately by the streaming-latency notes per model (Kyutai 0.5 s, Nemotron 80 ms–1.12 s, etc.).

### Q4 (2026-05-11): If RTFx is high, is latency automatically low?

Usually but not always — architecture matters more than the raw RTFx number. Combinations:

| Streaming? | High RTFx? | Real-time latency | Example |
|---|---|---|---|
| Yes | Yes | **Best** | Parakeet-TDT-0.6B-v2 (streams + RTFx 3,386) |
| Yes | Modest | Still good | Kyutai stt-1b-en_fr (0.5 s streaming, RTFx ~88) |
| No | Yes | **Bad** — fast batch but waits for chunks | Whisper-turbo (RTFx 200, 30 s chunks) |
| No | No | Useless for live | Tortoise-style offline models |

Whisper-turbo and Cohere Transcribe have great RTFx but process audio in chunks — the user has to wait for the chunk to complete before *any* text appears. That kills live-conversation latency even with high throughput numbers.

### Q5 (2026-05-11): So streaming + high RTFx = low latency for real-time?

Exactly right.

- **Streaming** → text emitted incrementally as audio arrives (no waiting for chunk completion)
- **High RTFx** → each incremental piece is computed fast enough to stay ahead of the audio coming in, with headroom for load spikes and weaker-than-A100 hardware
- Together → text appears almost as soon as words are spoken

A streaming model with RTFx barely above 1× real-time can fall behind under load on a smaller GPU; high RTFx provides the safety margin.

### Q6 (2026-05-11): So I need streaming + high RTFx + low WER. Confirm?

**Confirmed.** Hard requirements for the STT stage of a real-time voice agent:

1. ✅ **Streaming** (incremental emission, not chunk-based)
2. ✅ **High RTFx** (with headroom for load + non-A100 hardware)
3. ✅ **Low WER** (clean text into the downstream LLM)

Top STT candidates that pass all three, to benchmark in Phase 2:

| Model | Streaming | RTFx | WER (LibriSpeech clean / Open-ASR mean) | License |
|---|---|---|---|---|
| **Parakeet-TDT-0.6B-v2** | TDT decoder | 3,386 | 1.69 / 6.05 | CC-BY-4.0 |
| **Kyutai stt-1b-en_fr** | 0.5 s + semantic VAD | ~88 (2.6B sibling) | — / — (1B variant not on Open-ASR; 2.6B sibling 1.70 / 6.40) | CC-BY-4.0 |
| **Nemotron-Speech-Streaming-EN-0.6B** | 80 ms – 1.12 s configurable | high | 2.32 / 6.93 @ 1.12 s | NVIDIA OML |
| **Qwen3-ASR-1.7B via vLLM** | vLLM streaming | 166 | 1.63 / — (not on Open-ASR English; FLEURS-multi 4.90) | Apache 2.0 |

### Q7 (2026-05-11): What do "TDT", "0.5 s + VAD", "80 ms – 1.12 s", and "vLLM" mean in the Streaming column? And what does "low" mean in the WER column?

**TDT (Token-and-Duration Transducer)** — A decoder architecture that predicts each text token together with its audio duration. Because it doesn't need to see the full utterance before committing tokens, streaming is a native property of the decoder, not a workaround. Trade-off: slightly higher WER than LLM-decoder designs but huge throughput gains. Used by Parakeet-TDT-0.6B-v2.

**0.5 s + semantic VAD (Kyutai stt-1b-en_fr)** — Text is committed ~500 ms after the corresponding speech. **VAD = Voice Activity Detector** (decides when the user has stopped speaking). Plain VAD looks at silence and tends to cut users off mid-thought; **semantic VAD** considers meaning + intonation and waits for actual end-of-thought. Critical for voice agents that need to not interrupt the user during natural pauses.

**80 ms – 1.12 s configurable (Nemotron-Speech-Streaming-EN-0.6B)** — A runtime knob to pick where on the latency–accuracy curve you want to sit: 80 ms is almost-instant emission with slightly higher WER; 1.12 s gives the best WER (6.93 mean) while still feeling conversational. Unusual flexibility — most models bake one latency point in.

**vLLM (Qwen3-ASR-1.7B)** — vLLM is a high-throughput LLM serving engine (https://github.com/vllm-project/vllm). Because Qwen3-ASR is architected as an LLM that outputs text tokens, it streams token-by-token through vLLM the same way a chat model does. **Caveat:** streaming here comes from the *serving runtime*, not a natively-streaming acoustic decoder, and WER degrades slightly in streaming mode (offline 2.69 → streaming 3.33 avg per the Qwen3-ASR model card).

**"low" in the WER column** — was a placeholder where the exact Open-ASR-Leaderboard mean WER wasn't published for that specific variant. WER itself means "lower = better" (% of words wrong). Concrete values now in the table:

- Kyutai stt-1b-en_fr: not on Open-ASR leaderboard. 2.6B sibling sits at LS-clean 1.70 / Open-ASR mean 6.40, so the 1B is expected slightly behind that.
- Qwen3-ASR-1.7B: LibriSpeech test-clean **1.63**, test-other 3.38, FLEURS multilingual avg **4.90** (best multilingual in the survey). Not on the standard Open-ASR English leaderboard but objectively excellent.

### Q8 (2026-05-11): Is there a way to test STT models without downloading them?

Yes — several paths, each with different trade-offs.

**1. Hugging Face Spaces (free, browser-based) — best starting point.** Most popular models have an interactive demo Space (mic input → live transcription, no install). Examples: `openai/whisper`, `nvidia/parakeet-tdt-0.6b-v2`. Search https://huggingface.co/spaces for any model name.

**2. Vendor-hosted live demos.**
- Kyutai STT / Moshi: https://unmute.sh/ (real-time streaming demo, best for testing the 0.5 s + semantic VAD claim) and https://moshi.chat/
- Alibaba Qwen audio: https://chat.qwen.ai/ (voice mode)
- NVIDIA NIM playgrounds (Parakeet, Canary, Nemotron): https://build.nvidia.com/
- Mistral Voxtral via Le Chat: https://chat.mistral.ai/

**3. Hugging Face Inference API.** Programmatic free tier:
```python
from huggingface_hub import InferenceClient
client = InferenceClient(model="openai/whisper-large-v3")
text = client.automatic_speech_recognition("audio.wav")
```
Rate-limited; paid Inference Endpoints for production.

**4. Replicate / Modal / Baseten (serverless, pay-per-call).** Run any HF model via API without owning a GPU. https://replicate.com/ · https://modal.com/ · https://www.baseten.co/

**5. Google Colab.** Free T4 GPU; model downloads to ephemeral Colab VM (not your local machine).

**Mapping for the four shortlisted streaming STT models:**

| Model | Easiest no-download test |
|---|---|
| Parakeet-TDT-0.6B-v2 | HF Space `nvidia/parakeet-tdt-0.6b-v2` or build.nvidia.com NIM |
| Kyutai stt-1b-en_fr | https://unmute.sh/ (live streaming) |
| Nemotron-Speech-Streaming-EN-0.6B | build.nvidia.com NIM endpoint |
| Qwen3-ASR-1.7B | HF Space or chat.qwen.ai |

**Critical caveat:** Hosted demos are great for **quality/feel** but **bad for true latency measurement** — network round-trip adds 50–300 ms that has nothing to do with the model. To validate the "Kyutai 0.5 s" or "Nemotron 80 ms" claims, you'll eventually need to run locally so the measured latency is the model's, not the internet's. Recommended Phase-2 workflow: **(a)** browse with hosted demos to eliminate models that fail on your audio, **(b)** then run only the 2–3 finalists locally for real latency benchmarks on target hardware.
